 ...
 * @file   Transportee.c
 * @brief  arquivo de manipula��o de dados dos transportes
 * 
 * @author italo
 * @date   March 2023
...
bool RemoverTransporte(ListaTransporte** lista, ListaTransporte* transporteAtual) {
	...
	// Verifica se o transporte atual � nulo
	...
	// Libera a mem�ria do transporte atual
...
bool TrocarTransporte(ListaTransporte* transporte1, ListaTransporte* transporte2) {
	...
	// Troca os dados dos transportes usando uma vari�vel auxiliar
...
bool OrganizarTransportePorBateria(ListaTransporte* lista) {
	...
	// Ordena a lista de transportes por vida �til da bateria usando o algoritmo Bubble Sort
	while (!terminou)
	{
		terminou = true;
		listaAtual = lista;
...
ListaTransporte* ListarPorGeocode(ListaTransporte* lista, char geocode[STRG]) {
	...
	while (listaAtual != NULL)
	{
		if (strcmp(listaAtual->transporte.geocode, geocode) == 0) {
			// AdicionarLocal(&listaGeocode, listaAtual->transporte); // Erro: tipos incompatíveis
		}
		listaAtual = listaAtual->prox;
	}
